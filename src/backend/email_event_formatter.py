"""
Email Event Formatter - Dict-based orchestration event processing.

Converts orchestration events into OpenAI/LiteLLM message dicts for Email
agent conversations.  No LangChain dependency -- all output is plain dicts.

Ported from the LangChain-based EmailEventFormatter in chask_lambdas.
"""
...
EVENT_PREFIXES: Dict[str, Tuple[str, str]] = {
    ...
    "message_to_agent": ("[Operador]", "user"),
    "message_to_email_agent": ("[Mensaje del operador]", "user"),
...
class EmailEventFormatter:
    ...
    EVENT_HANDLERS: Dict[str, str] = {
        ...
        "message_to_email_agent": "_handle_regular_message",
    ...
    def format_events(
        ...
        for evt in sorted_events:
            ...
            if event_type not in _TOOL_EVENT_TYPES:
                ...
                content_key = (event_type, call_id) if call_id else None
    ...
    def _handle_batch_tool_execution(
        ...
        for tc in tool_calls:
            ...
            # Skip duplicate batch events — already registered from a prior
            # batch_tool_execution with the same tool_calls.
            if call_id in state["look_ahead"]:
                continue
    ...
    def _handle_function_call(
        ...
        """Handle single function_call events."""
        extra = evt.get("extra_params") or {}
        ...
        tool_calls = extra.get("tool_calls", [])
        if not tool_calls:
            logger.info(f"function_call event has no tool_calls in extra_params, keys: {list(extra.keys())}")
            return []
        ...
        logger.info(f"Registering function_call in look_ahead: call_id={call_id}, tool={tool_call.get('name')}")
    ...
    def _handle_tool_response(
        ...
        if not matched_id:
            logger.warning(
                f"Skipping tool response (no matching agent call): {tool_name}, "
                f"call_id={call_id}, look_ahead_keys={list(state['look_ahead'].keys())}"
            )
            return []

        if matched_id not in state["look_ahead"] or not state["look_ahead"][matched_id]:
            logger.warning(f"look_ahead stale for matched_id={matched_id}, skipping")
            return []
    ...
    def _handle_tool_error(
        ...
        if matched_id and matched_id in state["look_ahead"] and state["look_ahead"][matched_id]:
            call_data = state["look_ahead"][matched_id].pop(0)
            tool_call = call_data.get("tool_call")
            from_batch = call_data.get("from_batch", False)
    ...
    def _match_tool_call(cls, call_id: Optional[str], state: Dict) -> Optional[str]:
        ...
        # Fallback: find first pending batch ID that still has a valid look_ahead entry
        for bid in state["pending_batch_ids"]:
            if bid in state["look_ahead"] and state["look_ahead"][bid]:
                return bid
        return None
